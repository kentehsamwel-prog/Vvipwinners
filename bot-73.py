...
async def feedback_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    NAMES = ["James","Ali","Sarah","Ravi","Carlos","Fatou","Mike","Zara","John K","Patel",
             ...
             "Anna","Bilal","Emma","Vikram","Nadia","Joel"]

    def win_comment():
        return random.choice([
            f"Session today was fire! Won ${random.randint(120,4500)} 🔥",
            f"{acc_str} won today, signals were clean 💪",
            f"Today {acc_pct}% accuracy! Every trade was perfect 👑",
            f"Won ${random.randint(150,3200)} today following every signal 🙏",
            f"{wins} out of {total} won today. EVALON never disappoints 🏆",
            f"I joined today and I can't believe it! {wins} wins 😱",
            f"${random.randint(120,4500)} profit today. This is real! 🔥",
            f"Leo session ilikuwa nzuri! signals {wins}/{total} ziligonga 🏆",
            f"Nimeweza kupata faida! ${random.randint(120,1500)} kwa session moja 💰",
            f"Mashallah! Aaj sab signals perfect the 💰",
            f"Aaj {wins} signal sahi nikle, ${random.randint(890,3000)} profit hua 💪",
            f"Hoje foi incrível! {wins} de {total} acertaram 👑",
        ])
    ...
    for _ in range(random.randint(12, 18)):
        ...
        if random.random() < 0.3:
            ...
            dname = f"{name.split()[0]} {second.split()[0][0]}"
        else:
            dname = name.split()[0]
        fake_entries.append({"num": get_num(), "name": dname,
                              "stars": "⭐"*random.choice(stars_pool), "comment": win_comment()})

    real_entries = [f for f in load_feedback() if f.get("rating", 0) >= 4]
    all_entries  = [{"num": random.randint(1501,2800), "name": f.get("name","User"),
                     "stars": "⭐"*f.get("rating",5), "comment": f.get("comment","Great signals!")}
                    for f in real_entries] + fake_entries

    if not all_entries: await update.message.reply_text("📊 No feedback yet."); return

    # FIX 2: no real/generated count shown
    ...
    for entry in all_entries:
        try:
            await context.bot.send_message(chat_id=update.effective_chat.id,
                ...
                parse_mode="Markdown")
        except Exception as e: logger.warning(f"Feedback send failed: {e}")
        await asyncio.sleep(random.uniform(1.5, 3.0))

    await context.bot.send_message(chat_id=update.effective_chat.id,
        text="✅ *Done!*", parse_mode="Markdown",
