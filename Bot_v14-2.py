...
async def cmd_channelfeedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    SHORT = [
        ...
        f"${amt1} made today. Thank you 🙏", f"On point as always bro 🎯",
        f"${amt1} profit today. Simple 💰", f"Boss you killed it today 👊",
        f"Clean session today king 👑", f"Evalon never misses bro 🎯",
        f"This Evalon thing is real king 💎", f"Every signal landed today bro 🔥",
        f"${amt1} richer after today's session", f"Accuracy {acc_pct}% today. Wild 👑",
        f"Bro {wins} out of {total}. Crazy 💪", f"Never seen accuracy like this bro",
        f"${amt1} in the bag today king 🔥", f"Evalon is different bro, fr 💎",
        f"Signals on point today. ${amt1} profit", f"King every trade hit today 💪",
        f"Bro I was ready and it paid off. ${amt1} 🔥", f"No cap {acc_pct}% accuracy today 👑",
        f"Not one loss today bro 🎯", f"${amt1} and it's not even afternoon 💰",
        f"Bro Evalon hits different every time 🔥", f"Session was clean start to finish king 👑",
        f"Every entry was spot on today bro 💪", f"${amt1} made. Simple follow and profit 🎯",
        f"This accuracy is unreal bro. ${amt1} 💎", f"Followed every signal. ${amt1} in profit 🙏",
        f"Kaka leo ilikuwa moto 🔥", f"Asante sana bro, faida nzuri leo",
        f"Bhai aaj toh kamaal tha 🔥", f"Shukriya bhai, ${amt1} profit mila 🙏",
        f"Merci chef, {wins} sur {total} 👌", f"Perfeito hoje irmão, ${amt1} 💪",
    ]

    LONG = [
        f"Bro I have been trading for 2 years and never seen accuracy like this. Made ${amt2} today just following the signals. Every single one hit. King you are built different 👑",
        f"Evalon brother I was skeptical at first. But {acc_str} signals won today and I made ${amt1}. This is the real deal. No more guessing 💪",
        f"I told my friend about this after making ${amt1} today. He didn't believe me so I showed him my account. Now he wants to join too 😂 Accuracy was {acc_pct}% king 👊",
        f"I nearly gave up trading last month after losing money elsewhere. Today I made ${amt1} and I finally feel confident again. Every signal was precise bro. Thank you for real 🙏",
        f"Honestly the consistency is what gets me every time. Session after session, {acc_pct}% accuracy. Made ${amt1} today and I am not even using big amounts yet 💰",
        f"Brother I screenshotted my balance after today's session. ${amt2} in profit. Evalon is changing lives king, for real 🙏🔥",
        f"Bro {acc_pct}% accuracy today. I have tried 3 other signal groups before. None of them come close to this. ${amt1} profit and I am happy 💪",
        f"King this is the most consistent signal I have ever followed. Today {acc_str} won and I made ${amt1}. My trading changed completely since I joined 🔥",
        f"Man I used to trade randomly and lose. Now I just wait for the signal and follow it. ${amt1} profit today. Discipline is key bro 💪",
        f"I joined last week and already made back what I lost in 3 months elsewhere. Today was {acc_pct}% accuracy and ${amt1} profit. Evalon is built different king 💎",
        f"Bro I follow every signal without hesitation now. Today {acc_str} won and I cleared ${amt1}. Trust the process and it pays every time 🎯",
        f"Started with small amounts just to test. After today's {acc_pct}% accuracy and ${amt1} profit I am going bigger next session. King you never miss 👑",
        f"Bhai pehle main bahut loss karta tha dusri jagah se. Aaj {wins} mein se {wins} win hua. ${amt1} profit. Evalon ka level alag hai sach mein 🙏",
        f"Irmão hoje foi sensacional. {acc_str} sinais certos e ${amt2} de lucro. Obrigado mesmo 👑",
        f"Nimekuwa nikifuata signals kwa wiki mbili sasa. Kila session inanipa faida. Leo ${amt1} tena. Asante 🙏",
    ]
...
async def _send_help(chat_id, context):
    ...
    await context.bot.send_message(chat_id=chat_id, parse_mode="Markdown", text=(
        ...
        "`/feedback` — Send feedback in bot\n"
        "`/channelfeedback` — Select & forward feedback to channel\n"
        "`/reviewfeedback` — Approve/reject real member feedback\n"
