...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("fb_waiting"):
        ...
        # Notify admin â€” simple ping, full review via /reviewfeedback
        try:
            await context.bot.send_message(
                chat_id=ADMIN_ID,
                text=(
                    f"ðŸ“© *New Feedback â€” {name}*\n\n"
                    f"{'â­' * rating}\n"
                    f"ðŸ’¬ _{comment}_\n\n"
...
async def feedback_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    JOINED_TODAY = [
        ...
        f"Joined today. Already made ${amt1} just following the signals. No joke",
    ]
    ...
    SHORT = [
        ...
        f"${amt1} made just today. Thank you",
        f"On point as always bro",
        f"${amt1} profit. Simple and clean",
        ...
        f"${amt1} richer after this session",
        f"Accuracy {acc_pct}% today. Unreal",
        f"Bro {wins} out of {total}. Impressive",
        f"Never seen this kind of accuracy before",
        f"${amt1} in the bag today",
        f"Evalon hits different every time",
        f"Signals on point. ${amt1} profit",
        f"Every trade hit today king",
        f"Was ready and it paid off. ${amt1}",
        f"No cap {acc_pct}% accuracy today",
        f"Not one loss today",
        f"${amt1} and the session is still fresh",
        f"Consistency is the key here king",
        f"Session was perfect today",
        f"Every entry was spot on",
        f"${amt1} made. Follow the signal and profit",
        f"This accuracy is something else. ${amt1}",
        f"Followed every signal. ${amt1} in profit",
        f"Another solid session king",
        f"Bro {acc_pct}% is no joke today",
        f"${amt1} secured. Thank you",
        f"Results speak for themselves today",
        f"King you are too consistent",
        f"Profit again today. ${amt1} clean",
        f"Session was fire today",
        f"Every signal confirmed. ${amt1}",
        f"This is why I renewed my VIP. ${amt1}",
        f"Locked in and made ${amt1} today",
        f"Discipline plus Evalon equals profit",
        f"${amt1} just from following instructions",
        f"Today was effortless. ${amt1}",
        f"Bro I keep making money here",
        f"King I appreciate the consistency",
        f"Another day another profit. ${amt1}",
        f"Evalon never lets me down",
        f"${amt1} is a good day for me",
        f"Accuracy was top tier today",
        f"Boss session was on fire",
    ]

    LONG = [
        f"I have been trading for 2 years and never seen accuracy like this. "
        f"Made ${amt2} today just following the signals. Every single one hit. King you are built different",

        f"I was skeptical at first. "
        ...
        f"He did not believe me so I showed him my account. "
        f"Now he wants to join too. Accuracy was {acc_pct}%",

        f"I nearly gave up trading last month after losing elsewhere. "
        f"Today I made ${amt1} and I finally feel confident again. "
        f"Every signal was precise. Thank you for real",

        f"The consistency is what gets me every time. "
        f"Session after session {acc_pct}% accuracy. "
        f"Made ${amt1} today and I am not even using big amounts yet",

        f"I screenshotted my balance after today. "
        f"${amt2} in profit. Evalon is changing lives for real",

        f"{acc_pct}% accuracy today. I have tried 3 other signal groups before. "
        f"None of them come close to this. ${amt1} profit and I am happy",

        f"This is the most consistent signal I have ever followed. "
        f"Today {acc_str} won and I made ${amt1}. "
        f"My trading changed completely since I joined",

        f"I used to trade randomly and lose. "
        f"Now I just wait for the signal and follow it. "
        f"${amt1} profit today. Discipline is key",

        f"I joined last week and already made back what I lost in 3 months elsewhere. "
        f"Today was {acc_pct}% accuracy and ${amt1} profit. "
        f"Evalon is built different",

        f"I follow every signal without hesitation now. "
        f"Today {acc_str} won and I cleared ${amt1}. "
        f"Trust the process and it pays every time",

        f"Started with small amounts just to test. "
        f"After today's {acc_pct}% accuracy and ${amt1} profit I am going bigger next session. "
        f"King you never miss",

        f"My brother recommended Evalon and I thought it was just another group. "
        f"After today making ${amt1} with {acc_pct}% accuracy I am a believer. "
        f"This is different",

        f"I wake up ready because I know the signals are coming. "
        f"Today {acc_str} hit and I walked away with ${amt1}. "
        f"Best decision I made joining this group",

        f"Three months with Evalon and I have not had a bad week yet. "
        f"Today alone ${amt1} profit with {acc_pct}% accuracy. "
        f"King keep it up",

        f"People ask me where I get my signals from. "
        f"I just smile and stay quiet. "
        f"${amt2} today says everything",

        f"I used to overthink every trade. "
        f"Now I just wait for the signal open and close. "
        f"${amt1} made today with zero stress",

        f"Evalon taught me patience pays. "
        f"Waited for each signal today and made ${amt1}. "
        f"Every entry was clean",

        f"My account has grown every single week since joining. "
        f"Today {acc_pct}% accuracy and ${amt1} profit. "
        f"This is sustainable trading",

        f"I show my daily profits to my family now. "
        f"Today ${amt2} just from following signals. "
        f"They stopped doubting me",
    ]
    ...
    # ORDER: 3 fake â†’ real interleaved â†’ remaining fake
...
async def cmd_channelfeedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    JOINED_TODAY = [
        f"Joined today and already ${amt1} up. This is crazy ðŸ˜±",
    ...
    SHORT = [
        ...
        f"${amt1} made today. Thank you ðŸ™", f"On point as always bro ðŸŽ¯",
        f"${amt1} profit today. Simple ðŸ’°", f"Boss you killed it today ðŸ‘Š",
        ...
        f"${amt1} richer after today's session", f"Accuracy {acc_pct}% today. Wild ðŸ‘‘",
        f"Bro {wins} out of {total}. Crazy ðŸ’ª", f"Never seen accuracy like this bro",
        f"${amt1} in the bag today king ðŸ”¥", f"Evalon is different bro, fr ðŸ’Ž",
        f"Signals on point today. ${amt1} profit", f"King every trade hit today ðŸ’ª",
        f"Bro I was ready and it paid off. ${amt1} ðŸ”¥", f"No cap {acc_pct}% accuracy today ðŸ‘‘",
        f"Not one loss today bro ðŸŽ¯", f"${amt1} and it's not even afternoon ðŸ’°",
        f"Bro Evalon hits different every time ðŸ”¥", f"Session was clean start to finish king ðŸ‘‘",
        f"Every entry was spot on today bro ðŸ’ª", f"${amt1} made. Simple follow and profit ðŸŽ¯",
        f"This accuracy is unreal bro. ${amt1} ðŸ’Ž", f"Followed every signal. ${amt1} in profit ðŸ™",
        f"Kaka leo ilikuwa moto ðŸ”¥", f"Asante sana bro, faida nzuri leo",
        f"Bhai aaj toh kamaal tha ðŸ”¥", f"Shukriya bhai, ${amt1} profit mila ðŸ™",
        f"Merci chef, {wins} sur {total} ðŸ‘Œ", f"Perfeito hoje irmÃ£o, ${amt1} ðŸ’ª",
    ]

    LONG = [
        f"Bro I have been trading for 2 years and never seen accuracy like this. Made ${amt2} today just following the signals. Every single one hit. King you are built different ðŸ‘‘",
        f"Evalon brother I was skeptical at first. But {acc_str} signals won today and I made ${amt1}. This is the real deal. No more guessing ðŸ’ª",
        f"I told my friend about this after making ${amt1} today. He didn't believe me so I showed him my account. Now he wants to join too ðŸ˜‚ Accuracy was {acc_pct}% king ðŸ‘Š",
        f"I nearly gave up trading last month after losing money elsewhere. Today I made ${amt1} and I finally feel confident again. Every signal was precise bro. Thank you for real ðŸ™",
        f"Honestly the consistency is what gets me every time. Session after session, {acc_pct}% accuracy. Made ${amt1} today and I am not even using big amounts yet ðŸ’°",
        f"Brother I screenshotted my balance after today's session. ${amt2} in profit. Evalon is changing lives king, for real ðŸ™ðŸ”¥",
        f"Bro {acc_pct}% accuracy today. I have tried 3 other signal groups before. None of them come close to this. ${amt1} profit and I am happy ðŸ’ª",
        f"King this is the most consistent signal I have ever followed. Today {acc_str} won and I made ${amt1}. My trading changed completely since I joined ðŸ”¥",
        f"Man I used to trade randomly and lose. Now I just wait for the signal and follow it. ${amt1} profit today. Discipline is key bro ðŸ’ª",
        f"I joined last week and already made back what I lost in 3 months elsewhere. Today was {acc_pct}% accuracy and ${amt1} profit. Evalon is built different king ðŸ’Ž",
        f"Bro I follow every signal without hesitation now. Today {acc_str} won and I cleared ${amt1}. Trust the process and it pays every time ðŸŽ¯",
        f"Started with small amounts just to test. After today's {acc_pct}% accuracy and ${amt1} profit I am going bigger next session. King you never miss ðŸ‘‘",
        f"Bhai pehle main bahut loss karta tha dusri jagah se. Aaj {wins} mein se {wins} win hua. ${amt1} profit. Evalon ka level alag hai sach mein ðŸ™",
        f"IrmÃ£o hoje foi sensacional. {acc_str} sinais certos e ${amt2} de lucro. Obrigado mesmo ðŸ‘‘",
        f"Nimekuwa nikifuata signals kwa wiki mbili sasa. Kila session inanipa faida. Leo ${amt1} tena. Asante ðŸ™",
    ]
...
# ============================================================
# /realfeedback â€” see only real feedback with approval status
# ============================================================
async def cmd_realfeedback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id): return
    fb_list = load_feedback()
    if not fb_list:
        await update.message.reply_text("ðŸ“Š No real feedback yet."); return
...
async def _send_help(chat_id, context):
    await context.bot.send_message(chat_id=chat_id, parse_mode="Markdown", text=(
        "\U0001f4d6 *EVALON VIP SIGNALS â€” ADMIN GUIDE*\n\n"
        "â”â”â”â”â”â”â”â”â”â”"+"\n\U0001f4e1 *SIGNALS*\n"+"â”â”â”â”â”â”â”â”â”â”"+"\n"
        "`EURUSD 5` â†’ 1 trade (default)\n"
        "`EURUSD 5 10` â†’ 10 trades auto\n\n"
        "PREPARING â†’ BUY/SELL/Cancel\n"
        "If trades=1 â†’ bot asks count\n"
        "If trades>1 â†’ result sent auto\n\n"
        "â”â”â”â”â”â”â”â”â”â”"+"\n\U0001f4c5 *SESSION*\n"+"â”â”â”â”â”â”â”â”â”â”"+"\n"
        "`/session` â€” 30min/1hr alert\n"
        "`/end` â€” End (VIP only)\n\n"
        "â”â”â”â”â”â”â”â”â”â”"+"\n\U0001f4e2 *BROADCAST*\n"+"â”â”â”â”â”â”â”â”â”â”"+"\n"
        "Send photo/video â†’ direct to VIP\n"
        "`/broadcast text` â†’ VIP\n"
        "`/broadcast all text` â†’ Everyone\n"
    ))
    await context.bot.send_message(chat_id=chat_id, parse_mode="Markdown", text=(
        "â”â”â”â”â”â”â”â”â”â”"+"\n\U0001f511 *VIP CODES*\n"+"â”â”â”â”â”â”â”â”â”â”"+"\n"
        "`/addcode 1w Name` â€” 1 Week code\n"
        "`/addcode 1m Name` â€” 1 Month code\n"
        "`/addcode 3m Name` â€” 3 Months code\n"
        "`/addcode 6m Name` â€” 6 Months code\n"
        "`/addcode 1y Name` â€” 1 Year code\n"
        "`/addcodes 10 1m` â€” 10 codes (1 Month)\n"
        "`/listcodes` â€” View codes\n"
        "`/vipusers` â€” View VIP\n"
        "`/revoke USER_ID` â€” Remove VIP\n\n"
        "â”â”â”â”â”â”â”â”â”â”"+"\n\U0001f4ca *STATS & FEEDBACK*\n"+"â”â”â”â”â”â”â”â”â”â”"+"\n"
        "`/feedback` â€” Send feedback in bot\n"
        "`/channelfeedback` â€” Select & forward feedback to channel\n"
        "`/reviewfeedback` â€” Approve/reject real member feedback\n"
        "`/realfeedback` â€” View real feedback status\n"
        "`/stats` â€” Full statistics\n"
        "`/dbstatus` â€” Database health\n\n"
        "â”â”â”â”â”â”â”â”â”â”"+"\n\U0001f4ce *FILE IDs*\n"+"â”â”â”â”â”â”â”â”â”â”"+"\n"
        "`/getid` â†’ send sticker/photo â†’ get file\\_id\n"
    ))
