...
async def _do_expiry_check(bot):
    ...
    for uid_str, udata in list(db["users"].items()):
        ...
        if days_left <= 0:
            ...
            try:
                await bot.send_message(chat_id=uid,
                    text=f"⏰ *Heads up, {name}!*\n\nYour *VIP access expires in 3 days* (*{expiry_str}*).\n\nRenew now to keep receiving signals!\n\n{KAULI_MBIU}",
                    parse_mode="Markdown")
            except: pass
        elif days_left == 1:
            try:
                await bot.send_message(chat_id=uid,
                    text=f"🚨 *Last Warning, {name}!*\n\nYour *VIP access expires TOMORROW* (*{expiry_str}*).\n\n⚡ Renew *today* to avoid losing access!\n\n{KAULI_MBIU}",
                    parse_mode="Markdown")
            except: pass
...
def main():
    ...
    print("="*55)
    print("  EVALON VIP SIGNALS BOT v8")
    print("="*55)
