...
Fixes: 1-9 applied + Supabase persistent storage
"""
...
def _sb_get(key):
    try:
        res = supabase.table("kv_store").select("value").eq("key", key).execute()
        if res.data:
            return json.loads(res.data[0]["value"])
    except Exception as e:
        logger.warning(f"Supabase GET {key}: {e}")
    return None
...
def _sb_set(key, value):
    try:
        ...
        logger.warning(f"Supabase SET {key}: {e}")
        return False
...
def activate_code(code, uid, name):
    ...
    db["codes"][code].update({"used": True, "used_by": str(uid), "used_name": name,
                               "used_date": datetime.now().strftime("%Y-%m-%d %H:%M")})
    key = str(uid)
...
def new_code(label):
    ...
    db = load_db()
    db["codes"][code] = {"label": label, "used": False, "used_by": None,
                          "created": datetime.now().strftime("%Y-%m-%d %H:%M")}
    save_db(db); return code
...
# FIX 4: session end for VIP only
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # FIX 1: admin sees minimal panel
    if is_admin(uid):
        db_type = "✅ Supabase" if supabase else "⚠️ Local JSON"
        display = get_display_count()
        await update.message.reply_text(
            f"⚡ *EVALON VIP SIGNALS*\n"
            f"💾 {db_type}    👤 Total VIP Members: *{display}*",
            parse_mode="Markdown",
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("view_fb_"):
        ...
        sfb        = [f for f in fb_list if f.get("session_id") == session_id]
        if not sfb: await q.answer("No feedback received yet.", show_alert=True); return
        lines   = [f"📊 *SESSION FEEDBACK ({len(sfb)} responses):*\n"]
        ratings = [f["rating"] for f in sfb]
        lines.append(f"⭐ Average: *{sum(ratings)/len(ratings):.1f} / 5*\n")
        for i, fb in enumerate(sfb, 1):
            lines.append(f"{i}. ⭐ *{fb['rating']}/5* — {fb['name']}\n   💬 _{fb.get('comment','No comment')}_")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid):
        ...
        if context.user_data.get("awaiting_trade_count"):
            try: count = int(text.strip())
            except ValueError:
                await update.message.reply_text("Please enter a valid number (e.g. `10`)", parse_mode="Markdown"); return
            ...
            await _process_result(update, context, result, sig_id, count); return
...
async def handle_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.photo:
        ...
        sent, _ = await send_to_list(context, vip_ids, video=msg.video.file_id, caption=msg.caption)
    elif msg.animation:
...
async def broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if replied:
        ...
        elif replied.video:   sent, _ = await send_to_list(context, targets, video=replied.video.file_id, caption=replied.caption or caption or None)
...
async def feedback_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    NAMES = [
        "James","Ali","Sarah","Ravi","Carlos","Fatou","Mike","Zara","John K","Patel",
        "Sarah M","Ali B","Priya","Tariq","David","Marco","Aisha","Kevin","Fatima",
        "Raj S","Chris","Hassan","Nina","Pedro","Amara","Tony","Leila","Sam","Yusuf",
        "Kate","Arjun","Layla","Eric","Sofia","Ahmed","Grace","Omar","Lisa","Karim",
        "Anna","Bilal","Emma","Vikram","Nadia","Joel","Hamid","Lena","Ivan","Musa",
        "Tina","Felix","Zaid","Rita","Kwame","Sana","Bruno","Dina","Kofi","Mia"
    ]

    # ── Varied comment pool ──────────────────────────────────
    # "joined today" — only 3 in pool, added once below
    JOINED_TODAY = [
        f"Joined today bro and already ${amt1} up. This is crazy 😱",
        f"First day here and {wins} out of {total} won. Can't believe it king 😱",
        f"Leo ni siku yangu ya kwanza. Signals {acc_str} ziligonga. Asante kaka 😱",
    ]

    SHORT = [
        # Mix — some mention Evalon, some don't
        ...
        f"Ndugu signals zote ziligonga leo 💪",
        ...
        f"Perfeito hoje irmão, ${amt1} lucro 💪",
        f"Evalon never misses bro 🎯",
        f"Evalon kaka asante, leo nzuri sana 🏆",
        f"This Evalon thing is real king 💎",
        f"Evalon bhai aaj bhi perfect tha 👑",
    ]

    LONG = [
        f"Bro I have been trading for 2 years and never seen accuracy like this. "
        f"Today alone I made ${amt2} just following the signals. "
        f"Every single one hit. King you are built different 👑🔥",

        f"Evalon brother when I first joined I was skeptical. "
        f"But {acc_str} signals won today and I made ${random.randint(200,1500)} in one session. "
        f"This is the real deal. No more guessing 💪",

        f"I told my friend about this today after making ${amt1}. "
        f"He didn't believe me so I showed him my account. "
        f"Now he wants to join too 😂 Accuracy was {acc_pct}% today king 👊",

        f"Kaka leo ilikuwa tofauti kabisa. Signals {acc_str} ziligonga zote. "
        f"Nimefanya ${amt1} tu kwa masaa mawili. "
        f"Hii ni kitu kingine kabisa bro, asante sana 🏆",

        f"Ndugu nilikuwa na wasiwasi leo asubuhi lakini baada ya session hii "
        f"nimebadilisha mawazo yangu kabisa. Faida ya ${amt1} leo. "
        f"Evalon kaka unaujua mchezo huu 🔥",

        f"Bhai pehle main bahut loss karta tha dusri jagah se. "
        f"Aaj {wins} mein se {wins} win hua. ${random.randint(300,2000)} profit. "
        ...
        f"Today I made ${random.randint(150,800)} and I finally feel confident again. "
        ...
        f"${amt2} in profit. Going to send to family back home. "
        f"Evalon is changing lives king, for real 🙏🔥",
        ...
        f"Kila session inanipa faida. Leo ${amt1} tena. Mungu akubariki ndugu 🙏",
    ]

    # ── Emojis for variety — 60% with emoji, 40% without ──
    EMOJIS = ["🔥","💪","👑","🏆","💰","🎯","😱","🙏","👊","✅","💎","⚡"]
    ...
    def make_name():
        ...
        if random.random() < 0.35:
            ...
            return f"{name.split()[0]} {second.split()[0][0]}"
        return name.split()[0]

    def make_fake(comment=None):
        emoji_icon = random.choice(["","","","🇰🇪","🇳🇬","🇮🇳","🇧🇷","🇵🇰","🇿🇦",""]) if random.random() < 0.3 else ""
        return {
            "num":     get_num(),
            "name":    make_name() + (" " + emoji_icon if emoji_icon else ""),
    ...
    # Add exactly 2 "joined today" spread across fake pool
    joined_pool   = random.sample(JOINED_TODAY, 2)
...
async def _send_help(chat_id, context):
    ...
    await context.bot.send_message(chat_id=chat_id, parse_mode="Markdown", text=(
        "━━━━━━━━━━"+"\n\U0001f511 *VIP CODES*\n"+"━━━━━━━━━━"+"\n"
        "`/addcode Label` — 1 code\n"
        "`/addcodes 10` — 10 codes\n"
...
async def cmd_dbstatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if supabase:
        try:
            ...
            await update.message.reply_text(f"❌ *Supabase Error!*\n\n`{e}`", parse_mode="Markdown")
    else:
        await update.message.reply_text("⚠️ *Supabase not connected!*\n\nSet `SUPABASE_URL` and `SUPABASE_KEY` on Render.", parse_mode="Markdown")
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines = [
        ...
        f"\n💾 Storage: *{'✅ Supabase' if supabase else '⚠️ Local JSON'}*\n",
        "\n━━━━━━━━━━━━━━",
...
def main():
    ...
    print("="*55)
    print(f"Storage  : {'Supabase ✅' if supabase else 'Local JSON ⚠️'}")
    db = load_db()
